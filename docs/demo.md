...
- Optional: [`cloudflared`](https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/) to run the connector after create
...
One-shot create + optional hostname routing. Default local URL is `http://localhost:8000`.
...
Then run the printed `cloudflared tunnel run --token …` command. If DNS was not created via API:
...
Capture the tunnel `id` from create output.
...
4. Keep the session under ~2 minutes: zones → dns:list → dns:create → tunnel:expose → tunnel:delete.
